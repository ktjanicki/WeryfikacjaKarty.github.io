const checkCardNumber = (cardNumber) => {
    if (typeof cardNumber !== 'string') {
        throw new Error('Wprowadzona wartość nie jest typu string');
    }
    if (new RegExp(/[a-z]/gi).test(cardNumber)) {
        throw new Error('Numer karty nie może zawierać liter');
    }
    if (cardNumber === '') {
        return 'Nic nie wprowadzono';
    }
    ...
    const luhnaCheck = (cardNumber) => {
        ...
        const multiplication = reverseCardDigits.map((num, index) =>
            !even(index) ? parseInt(num * 2) : parseInt(num)
        );
